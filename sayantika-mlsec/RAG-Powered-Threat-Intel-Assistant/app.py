...
EXAMPLE_QUERIES = [
    ...
    "How does an adversary defeat MFA, log in with legitimate credentials, then change account settings to keep persistent access?",
    "Which built-in Windows utilities do adversaries commonly abuse to decode or deobfuscate a payload before running it?",
]
...
with gr.Blocks(
    ...
    query_input = gr.Textbox(
        ...
        placeholder='e.g. "What are common lateral movement techniques?" or "How is T1566 detected?"',
        lines=3,
        max_lines=6,
        elem_id="query-input"
    )
