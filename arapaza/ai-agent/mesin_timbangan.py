"""
Mesin AI multi-agent untuk tim SALES & SERVICE produk TIMBANGAN (timbanganindonesia.com).

Domain: timbangan/weighing scales & load cell (badan, counting, gantung/crane, portable,
duduk, emas, harga, hewan, hybrid, laboratorium, lantai, platform, truk; load cell; indikator;
anak timbangan; junction box). Pola sama seperti HOBO/Fakopp/HMP: chat multi-turn + Router +
Compare + Riset Harga + Budget. Agent Product mengecek katalog timbanganindonesia.com.
"""
import re
import os
import base64
...
def _agent_reader_teks(chat):
    ...
    prompt = (
        "Kamu Agent Reader (teks) untuk tim sales & service TIMBANGAN (weighing scale & load cell). Baca chat "
        "customer, lalu ekstrak SEMUA info penting secara terstruktur (poin '-'): APA yang ditimbang, KAPASITAS "
        "maksimum (Max), KETELITIAN/pembacaan (d/e), JENIS timbangan yang dibutuhkan (badan, counting, gantung/crane, "
        "portable, duduk, emas, harga, hewan, hybrid, laboratorium/analytical, lantai/floor, platform, truk/jembatan "
        "timbang, dll), ukuran platform, lokasi/lingkungan (toko, industri, lab, outdoor, area basah), butuh LEGAL "
        "FOR TRADE/TERA (Metrologi Legal) atau tidak, kebutuhan indikator/printer/koneksi (RS232, USB, LAN, load cell, "
        "junction box), jumlah unit, budget/timeline bila ada, serta hal yang ambigu. JANGAN menyimpulkan solusi, "
...
def _agent_reader_visual(file_bytes, mime):
    ...
    prompt = (
        "Kamu Agent Reader (visual) untuk tim TIMBANGAN. Baca gambar/PDF/datasheet terlampir. Ekstrak info "
        "terstruktur (poin '-'): jenis dokumen (foto barang/lokasi, datasheet timbangan, spesifikasi/RKS, foto "
        "objek yang ditimbang), jenis & merek/model timbangan yang tampak, kapasitas & ketelitian tertera, ukuran "
        "platform, indikator/load cell terlihat, label/anotasi penting. Kalau ada bagian tidak terbaca, katakan "
        "jujur. Baca ulang & verifikasi tiap angka, tipe/kapasitas/ketelitian, dan label yang terlihat sebelum ditulis; utamakan BENAR walau lebih lama. JANGAN mengarang."
    )
    return _gen(prompt, file_bytes, mime)


# ── Agent Product (fokus katalog timbanganindonesia.com) ──────────────
def _agent_product(info, koreksi=""):
    fb = ("\n\n=== KOREKSI DARI CHECKER (WAJIB dipatuhi) ===\n" + koreksi) if koreksi else ""
    prompt = (
        "Kamu Agent Product untuk TIMBANGAN. Berdasarkan kebutuhan (dari Reader) di bawah, rekomendasikan produk "
        "timbangan yang tepat untuk tiap kebutuhan.\n"
        "UTAMAKAN mencari & memverifikasi dari katalog: " + TIMBANGAN_SITES + " (distributor timbangan). "
        "Sesuaikan JENIS timbangan dengan kebutuhan (mis. hitung jumlah barang -> timbangan counting; barang di "
        "pallet -> platform/lantai; kendaraan -> timbangan truk/jembatan timbang; lab -> analytical/precision "
        "balance; emas -> timbangan emas; gantung -> crane scale). Sebut merek/model yang lazim (mis. CAS, CASTON, "
        "Newtech, Fujitsu, Dini Argeo, dan indikator seperti CI-series) bila kamu yakin.\n"
        "Untuk tiap rekomendasi: nama produk/tipe + kapasitas & ketelitian yang cocok + fungsi + kenapa cocok. Sebut "
        "aksesori bila perlu (indikator, printer, load cell, junction box, tiang display). Kalau customer butuh untuk "
        "transaksi jual-beli, ingatkan soal TERA/legal-for-trade.\n"
        "Produk HARUS sesuai kebutuhan Reader (kapasitas & ketelitian masuk akal). Jangan mengarang model/seri yang "
        "tidak kamu yakini; kalau ragu sebut kategori & tandai 'perlu verifikasi katalog'.\n\n"
        "=== KEBUTUHAN (DARI READER) ===\n" + info + fb
    )
    return _gen_search(prompt)


# ── Agent Technical (spesifikasi, setup, kalibrasi, BOM) ──────────────
def _agent_technical(info, produk, koreksi=""):
    fb = ("\n\n=== KOREKSI DARI CHECKER (WAJIB dipatuhi) ===\n" + koreksi) if koreksi else ""
    prompt = (
        "Kamu Agent Technical TIMBANGAN. Dari kebutuhan + produk terpilih di bawah, susun:\n"
        "1. SPESIFIKASI & KONFIGURASI: kapasitas (Max) x ketelitian (d), ukuran platform, jenis & jumlah load cell, "
        "indikator + fitur (counting, printer, koneksi RS232/USB/LAN), material (stainless untuk area basah), sumber "
        "daya (baterai/adaptor).\n"
        "2. BILL OF MATERIALS: daftar unit (timbangan/platform, load cell, indikator, junction box, printer, aksesori) "
        "+ estimasi jumlah. Bila jumlah tidak pasti, beri estimasi + tulis asumsi.\n"
        "3. CATATAN TEKNIS: instalasi (pondasi untuk truk/jembatan timbang), kalibrasi & TERA/legal-for-trade, "
        "lingkungan (getaran, angin, suhu), kapasitas vs ketelitian (jangan over/under-range), keselamatan.\n"
...
# ── Agent Service (kalibrasi/tera, perawatan, troubleshooting) ────────
def _agent_service(info, produk, koreksi=""):
    fb = ("\n\n=== KOREKSI DARI CHECKER (WAJIB dipatuhi) ===\n" + koreksi) if koreksi else ""
    prompt = (
        "Kamu Agent Service untuk produk TIMBANGAN (after-sales). Dari kebutuhan + produk di bawah, berikan info "
        "service relevan:\n"
        "- KALIBRASI & TERA: timbangan untuk transaksi WAJIB tera/tera ulang (Metrologi Legal); jelaskan pentingnya, "
        "interval umum, dan kalibrasi berkala + sertifikat.\n"
        "- PERAWATAN: rawat load cell (hindari overload/kejut), kebersihan platform, proteksi kelembapan/air.\n"
        "- TROUBLESHOOTING: masalah umum (pembacaan tidak stabil/drift, error load cell, indikator, baterai, hasil "
        "beda dari acuan) + langkah diagnosa.\n"
        "- GARANSI: umum untuk produk timbangan + saran (konfirmasi kebijakan spesifik ke distributor).\n"
        "Kalau chat tidak menyangkut service, cukup beri tips singkat kalibrasi/tera & perawatan. Jangan mengarang "
        "kebijakan garansi spesifik.\n\n"
        "=== KEBUTUHAN ===\n" + info + "\n\n=== PRODUK ===\n" + produk + fb
    )
    return _gen(prompt)
...
def _agent_checker_all(teks_info, visual_info, produk, teknis, service):
    prompt = (
        "Kamu Agent Checker (KOORDINATOR) untuk tim TIMBANGAN. Pastikan SEMUA hasil agent SALING KONSISTEN dan sesuai "
        "apa yang dibaca Reader. Reader adalah acuan kebenaran kebutuhan customer.\n\n"
        "Periksa:\n"
        "- Apakah PRODUK yang direkomendasikan sesuai JENIS, KAPASITAS & KETELITIAN yang diminta Reader? (mis. butuh "
        "hitung barang -> counting; kapasitas 500 kg -> jangan tawarkan 3 kg). Salah jenis/kapasitas = tidak konsisten.\n"
        "- Apakah TECHNICAL (spesifikasi/BOM) sesuai kebutuhan & produk?\n"
        "- Apakah SERVICE relevan (mis. transaksi -> tera wajib disebut)?\n"
...
def _agent_flow(teknis):
    prompt = (
        "Kamu Agent Flow. Dari deskripsi spesifikasi/teknis di bawah, buat DIAGRAM ALUR (flowchart) sistem TIMBANGAN "
        "dalam sintaks MermaidJS.\n"
        "Aturan:\n"
        "- Baris pertama WAJIB: flowchart TD  (boleh LR bila lebih pas).\n"
        "- Node = komponen (platform/load cell, junction box, indikator, printer, PC, dll). Pakai ID pendek + label "
        "dalam kurung siku, mis. L1[Load Cell]. HINDARI tanda kutip, koma, titik dua, kurung bulat, karakter khusus "
        "di label.\n"
        "- Edge = aliran sinyal/data; beri label singkat bila perlu, mis. L1 -->|mV| JB1.\n"
...
def _agent_compare(info, produk):
    prompt = (
        "Kamu Agent Compare untuk tim sales TIMBANGAN. Berdasarkan kebutuhan + produk yang direkomendasikan di bawah, "
        ...
        "Cari & verifikasi di web. Contoh merek pembanding (sesuaikan jenis & prioritas asal): Mettler Toledo "
        "(Swiss/AS), Sartorius (Jerman), Ohaus (AS), Dini Argeo (Italia), Radwag (Polandia), Avery Weigh-Tronix "
        "(AS/Inggris); China: Yaohua/YHZ, dll; lalu merek lokal bila ada.\n"
        "Untuk tiap produk, beri 1-2 padanan + kenapa setara (kapasitas/ketelitian) + perbedaan utama. Buat TABEL "
        "Markdown: Kebutuhan | Produk (katalog) | Alternatif (merek+model+negara) | Perbedaan utama.\n"
...
def _agent_harga_riset(produk, compare):
    prompt = (
        "Kamu Agent Riset Harga. Tugasmu MENCARI HARGA PASAR TERKINI (real, hari ini) dari WEB untuk SETIAP produk "
        "timbangan di bawah — baik produk katalog maupun alternatif merek lain.\n"
        "Untuk tiap produk cari: harga jual ke END-USER, mata uang aslinya (IDR/USD/EUR), dan sumber/domain bila ada "
        "(toko resmi, distributor, marketplace). Timbangan rentang harganya LEBAR (dari ratusan ribu untuk timbangan "
        "kecil sampai ratusan juta untuk jembatan timbang truk) — sesuaikan dengan jenis & kapasitas, JANGAN menebak "
        "terlalu murah untuk alat besar/presisi tinggi.\n"
...
def _agent_budget(info, produk, compare, harga):
    prompt = (
        "Kamu Agent Budget untuk Taharica (distributor timbangan). Basiskan estimasi pada HARGA PASAR hasil riset di "
        "bawah (jangan mengarang angka baru yang jauh berbeda).\n"
        "LOGIKA HARGA (WAJIB, jangan sampai under-price):\n"
        "- HARGA PENAWARAN ke customer ≈ HARGA PASAR end-user. Boleh sedikit di atas untuk layanan/kalibrasi/tera/"
        "instalasi. JANGAN menetapkan penawaran JAUH DI BAWAH harga pasar.\n"
        "- HARGA MODAL (biaya beli Taharica) = HARGA PENAWARAN DIKURANGI margin distributor ~15-35%. Jadi modal SELALU "
        "LEBIH KECIL dari penawaran, dan penawaran mendekati harga pasar.\n"
        "- Sanity check: modal < penawaran, penawaran tidak lebih rendah dari harga pasar tanpa alasan. Sebut mata "
        "uang & konversi Rp. Beri RENTANG. Tandai '(estimasi, wajib diverifikasi)'. Catat: instalasi/tera/kurs/qty "
        "dan kebijakan margin asli Taharica dapat mengubah angka.\n\n"
        "Hasilkan DUA bagian (mencakup produk katalog DAN alternatif, buat tabel: produk | harga pasar | modal | penawaran):\n"
...
def _agent_result(info, inkon, tanya, produk, teknis, service, compare, budget, instruksi=""):
    ...
    prompt = (
        "Kamu Agent Result untuk tim sales & service TIMBANGAN. Berdasarkan SELURUH data di bawah, buat TIGA output "
        "Bahasa Indonesia:\n"
        "1. output_awam_hobo: rekomendasi balasan untuk SALES memakai produk dari KATALOG (bahasa sederhana, siap "
        "dipakai membalas customer; sebut jenis/tipe timbangan + kapasitas/ketelitian + poin service/tera + kisaran "
        "harga penawaran bila relevan).\n"
        ...
        "3. output_technical: rangkuman teknis mendetail untuk tim teknik/service (produk katalog + alternatif, "
        "spesifikasi/BOM, kalibrasi/tera/instalasi, perbandingan teknis, dan ringkasan estimasi biaya modal vs penawaran).\n"
...
def _agent_router(chat, riwayat, prev):
    ringkas = (
        "Produk (katalog): " + _clip(prev.get("produk")) +
    ...
    prompt = (
        "Kamu Agent Router (dispatcher) untuk percakapan lanjutan tim sales TIMBANGAN. Ada HASIL ANALISA SEBELUMNYA "
        ...
        "reader (baca ulang kebutuhan), product (rekomendasi produk timbangan), technical (spesifikasi/BOM), "
        "service (kalibrasi/tera/after-sales), compare (alternatif merek lain), budget (estimasi harga).\n"
        "Aturan:\n"
        "- Pilih HANYA agent yang relevan dengan pesan baru. Contoh: 'ganti timbangannya' -> [product] (dan technical/"
        "budget bila jelas terpengaruh); 'hitung ulang harga' -> [budget]; 'carikan alternatif China' -> [compare]; "
        "'kapasitasnya jadi 500 kg' -> [product, technical, budget].\n"
...
def analisa_timbangan(chat, image_base64="", image_mime="image/png", riwayat="", sebelumnya=None):
    chat = (chat or "").strip()
