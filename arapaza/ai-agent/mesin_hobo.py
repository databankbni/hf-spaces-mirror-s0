...
def _agent_harga_riset(produk, compare):
    prompt = (
        ...
        "bawah — baik produk HOBO maupun alternatif merek lain.\n"
        "Untuk tiap produk cari: harga jual/list ke END-USER, mata uang aslinya (USD/EUR/dll), dan sumber/domain bila "
        "ada (mis. toko resmi, distributor, marketplace B2B).\n"
        "PENTING: alat ukur/logger ini sering PREMIUM. JANGAN menebak terlalu murah. Kalau angka pasti tak ketemu, beri "
        "RENTANG realistis berdasar produk sekelas + tandai 'perkiraan'. Konversikan kasar ke IDR (asumsi kurs USD≈Rp "
        "16.000, EUR≈Rp 17.500 — sebutkan asumsimu).\n"
...
def _agent_budget(info, produk, compare, harga):
    prompt = (
        ...
        "- HARGA PENAWARAN ke customer ≈ HARGA PASAR end-user (harga yang wajar dibayar customer). Boleh sedikit di "
        "atas untuk layanan/garansi lokal. JANGAN menetapkan penawaran JAUH DI BAWAH harga pasar.\n"
        "- HARGA MODAL (biaya beli Taharica sebagai distributor) = HARGA PENAWARAN DIKURANGI margin distributor "
        "~20-35%. Jadi modal SELALU LEBIH KECIL dari penawaran, dan penawaran mendekati harga pasar.\n"
        "- Sanity check: pastikan modal < penawaran, dan penawaran tidak lebih rendah dari harga pasar tanpa alasan. "
        "Sebut mata uang & konversi Rp. Beri RENTANG. Tandai '(estimasi, wajib diverifikasi)'. Catat: bea masuk, kurs, "
        "kuantitas, dan kebijakan margin asli Taharica dapat mengubah angka.\n\n"
        "Hasilkan DUA bagian (mencakup produk HOBO DAN alternatif, buat tabel: produk | harga pasar | modal | penawaran):\n"
