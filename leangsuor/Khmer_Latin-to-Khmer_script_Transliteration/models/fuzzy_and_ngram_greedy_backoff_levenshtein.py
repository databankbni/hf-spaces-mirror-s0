"""
Fuzzy N-gram Contextual Predictor (Levenshtein variant)
=======================================================

Identical to ``fuzzy_and_ngram_greedy_backoff`` in every respect EXCEPT the
fuzzy-matching similarity metric: this variant scores roman-word closeness with
a **normalized Levenshtein (edit-distance) similarity** instead of difflib's
``SequenceMatcher.ratio()``.

    similarity = 1 - levenshtein_distance(a, b) / max(len(a), len(b))

where ``levenshtein_distance`` is the minimum number of single-character
insertions, deletions, or substitutions to turn ``a`` into ``b``. The result is
in [0, 1]; 1.0 means the strings are identical.

Only :meth:`KhmerFuzzyContextualPredictor.similarity` changes. Candidate
generation, context scoring, beam search, and persistence are unchanged, so the
same ``fuzzy_threshold_base`` / ``fuzzy_threshold_context`` cut-offs apply — but
note the numeric scale differs slightly from SequenceMatcher, so thresholds may
warrant retuning.
...
---------------------
    ...
    2. CANDIDATE GENERATION
       ...
       - Fuzzy Matching:  Levenshtein similarity ≥ threshold (0.4–0.5)
                            ↓
    3. CONTEXT SCORING
...
# Prefer a fast C-backed Levenshtein if one is installed; otherwise fall back to
# a self-contained pure-Python implementation so the module has no hard
# dependency beyond the project defaults.
try:
    from rapidfuzz.distance import Levenshtein as _RFLevenshtein

    def _levenshtein_similarity(a, b):
        """Normalized Levenshtein similarity in [0, 1] via rapidfuzz."""
        return _RFLevenshtein.normalized_similarity(a, b)

except ImportError:  # pragma: no cover - exercised only without rapidfuzz

    def _levenshtein_distance(a, b):
        """Minimum single-character edits to turn ``a`` into ``b`` (pure Python)."""
        if a == b:
            return 0
        if not a:
            return len(b)
    ...
    def _levenshtein_similarity(a, b):
        """Normalized Levenshtein similarity in [0, 1] (pure Python)."""
        if not a and not b:
            return 1.0
...
class KhmerFuzzyContextualPredictor:
    ...
    def similarity(self, word1, word2):
        """Calculate similarity between two words using normalized Levenshtein.

        similarity = 1 - levenshtein_distance(word1, word2) / max(len1, len2)

        where levenshtein_distance is the minimum number of single-character
        insertions, deletions, or substitutions to turn one string into the
        other. Result is in [0, 1]; 1.0 means identical.
        """
        return _levenshtein_similarity(word1, word2)
    ...
    def find_candidates_with_context(
        ...
        fuzzy_matches.sort(
            key=lambda x: (x[1] * (1 + x[4])), reverse=True
        )
        candidates.extend(fuzzy_matches[:max_candidates])
        return candidates[:max_candidates]

    def predict_word_with_context(self, roman_word, context_words=None, top_k=5):
    ...
    def translate_with_beam_search(self, roman_sentence, beam_size=3):
        ...
        for i, roman_word in enumerate(roman_words):
            ...
            for hypothesis in beam:
                candidates = self.predict_word_with_context(
                    roman_word, context, top_k=beam_size
                )
                        ...
                        new_beam.append(
                            {
                                ...
                                "score": hypothesis["score"] + np.log(score + 1e-10),
                                "roman_processed": i + 1,
                            }
                        )
    ...
    def export(self, run_dir, model_name="fuzzy_ngram_greedy_backoff_levenshtein"):
        ...
        model_name : str
            Subdirectory name inside *run_dir*
            (default: ``fuzzy_ngram_greedy_backoff_levenshtein``).
        ...
        config = {
            "model": {
                "name": model_name,
                "type": "statistical",
                "algorithm": "fuzzy_ngram_greedy_backoff_levenshtein",
            },
            "hyperparameters": {
                "max_ngram": self.max_ngram,
