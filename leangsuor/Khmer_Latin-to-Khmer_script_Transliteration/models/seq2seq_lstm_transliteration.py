"""
Character-Level Seq2Seq LSTM for Khmer Latin → Khmer Script Transliteration
============================================================================
...
---------------------
    ...
    ┌─────────────────────────────────────────────────────────────┐
    │                        ENCODER                              │
    │  Roman chars → Embedding(128) → LSTM(256) → [h, c] states  │
    └─────────────────────────────────────────────────────────────┘
                              ↓
                     (hidden states passed as initial state)
                              ↓
    ┌─────────────────────────────────────────────────────────────┐
    │                        DECODER                              │
    │  Khmer chars → Embedding(128) → LSTM(256) → Dense(softmax) │
    │                     (init with encoder states)              │
    └─────────────────────────────────────────────────────────────┘

Data Preparation Pipeline (before the embedding layer)
------------------------------------------------------

    1. LOAD DATA
       JSON dict → parallel lists: khmer_script[], khmer_latin[]

    2. BUILD VOCABULARY
       Extract unique chars → char2idx / idx2char mappings
       Index 0 reserved for padding

    3. ADD SPECIAL TOKENS (target side only)
       \\t = start token,  \\n = end token

    4. ENCODE SEQUENCES
       chars → integer indices via char2idx lookup

    5. PAD / TRUNCATE
       Pad with 0s to max_src_len / max_tgt_len

    6. TRAIN / VAL SPLIT
       95 % train, 5 % validation (random_state=42)

Key design choices
~~~~~~~~~~~~~~~~~~
    - Character-level tokenisation (no subword segmentation)
    - Teacher forcing during training (decoder sees ground truth at t-1)
    - Autoregressive greedy decoding at inference
    - mask_zero=True in Embedding so padded positions are ignored
...
def load_config(config_path="config.ini"):
    """Load configuration from INI file, including [lstm] array settings."""
    ...
    if os.path.exists(config_path):
        ...
        if "lstm" in config:
            l = config["lstm"]
            defaults["data_path"] = l.get("data_path", defaults["data_path"])
            defaults["epochs"]    = l.getint("epochs", defaults["epochs"])
            if l.get("batch_sizes"):
                defaults["batch_sizes"] = _int_list(l["batch_sizes"])
            if l.get("embed_dims"):
                defaults["embed_dims"]  = _int_list(l["embed_dims"])
            if l.get("latent_dims"):
                defaults["latent_dims"] = _int_list(l["latent_dims"])
...
def _next_version_dir(base_output_dir, batch_size, embed_dim, latent_dim):
    ...
    version = 1
    while True:
        candidate = day_dir / f"lstm_v{version}_bs{batch_size}_ed{embed_dim}_ld{latent_dim}"
...
def build_vocab(seqs):
    ...
    Returns
    -------
    char2idx : dict   char → int  (1-indexed; 0 is reserved for padding)
...
def encode_tgt(seqs, char2idx, max_len, start_idx, end_idx):
    ...
    Returns
    -------
    decoder_input  : np.ndarray  (N, max_len)      shifted right (starts with <start>)
    decoder_target : np.ndarray  (N, max_len, 1)    ground truth  (ends with <end>)
    """
    decoder_input = []
    decoder_target = []
    for s in seqs:
...
def build_seq2seq_model(vocab_src, vocab_tgt, embed_dim=128, latent_dim=256):
    """Build encoder-decoder LSTM model and return training + inference models.

    Returns
    -------
    model          : training model   ([encoder_input, decoder_input] → decoder_output)
    encoder_model  : inference encoder (encoder_input → [state_h, state_c])
    decoder_model  : inference decoder ([decoder_input, state_h, state_c] → [output, h, c])
    """
    # --- Encoder ---
    ...
    encoder_lstm = layers.LSTM(latent_dim, return_state=True, use_cudnn=False, name="encoder_lstm")
    _encoder_outputs, state_h, state_c = encoder_lstm(x)
    ...
    decoder_lstm = layers.LSTM(
        latent_dim, return_sequences=True, return_state=True, use_cudnn=False, name="decoder_lstm"
    )
    ...
    # --- Inference: encoder ---
    encoder_model = models.Model(encoder_inputs, encoder_states, name="encoder_model")

    # --- Inference: decoder ---
    decoder_state_input_h = layers.Input(
        shape=(latent_dim,), name="decoder_state_input_h"
    )
    decoder_state_input_c = layers.Input(
        shape=(latent_dim,), name="decoder_state_input_c"
    )
...
class TrainingProgressCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch > 0:
            ...
            print(
                f"    Loss Change:     {loss_delta:+.4f} ({'↓' if loss_delta < 0 else '↑'})"
            )
            print(
                f"    Val Loss Change: {val_loss_delta:+.4f} ({'↓' if val_loss_delta < 0 else '↑'})"
            )
...
def train(
    ...
    run_dir : str
        ...
        ``outputs/27052026/lstm_v1_bs64_ed64_ld128``.
    ...
    dec_in_train, dec_in_val = (
        decoder_input_data[train_idx],
        decoder_input_data[val_idx],
    )
    dec_tgt_train, dec_tgt_val = (
        decoder_target_data[train_idx],
        decoder_target_data[val_idx],
    )
    ...
    with open(
        os.path.join(paths["evaluation"], "vocab_config.json"), "w", encoding="utf-8"
    ) as f:
    ...
    model_config = {
        "model": {
            "name": model_name,
            "architecture": "encoder_decoder_lstm",
            "embed_dim": embed_dim,
            "latent_dim": latent_dim,
        },
        "training": {
    ...
    print(
        f"  Best Validation Loss:  {history_dict['best_val_loss']:.4f} (Epoch {history_dict['best_epoch']})"
    )
    print(f"\nSaved to: {paths['root']}")
...
class Seq2SeqTransliterator:
    """Load saved encoder/decoder models and transliterate roman → Khmer.

    Parameters
    ----------
    model_dir : str
        Root directory for this model, e.g.
        ``outputs/03042026-143052/seq2seq_lstm``.
    ...
    def __init__(self, model_dir):
        ...
        print(f"Loaded LSTM models from {enc_path}")
    ...
    def predict_word(self, roman_word):
        ...
        with tf.device("/CPU:0"):
            states = self.encoder_model(X_in, training=False)
            ...
            while True:
                output_tokens, h, c = self.decoder_model(
                    [target_seq] + list(states), training=False
                )
                sampled_idx = int(output_tokens[0, -1, :].numpy().argmax())

                if (
                    sampled_idx in (self.end_token_idx, 0)
                    or len(decoded_chars) >= self.max_tgt_len
                ):
                    break
...
def corpus_bleu(references, hypotheses, max_n=4):
    ...
    return bp * math.exp(s) if s != 0.0 else 0.0
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train LSTM Seq2Seq model for Khmer Latin → Khmer Script transliteration"
    )
    ...
    print("\n" + "=" * 60)
    print("LSTM Seq2Seq — Multi-Version Training")
    print("=" * 60)
