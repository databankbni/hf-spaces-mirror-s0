...
from app.ingestion.domain.chunking import ChunkingResult, segment_document_with_stats
from app.ingestion.domain.models import Chunk, ManifestSource, ParsedDocument
from app.ingestion.application.parser import parse_source
...
class BaseSubjectPipeline:
    ...
    def content_contract(self) -> ContentContract:
        ...
        return get_category_policy(self.subject).content_contract
    ...
    def segment(self, document: ParsedDocument) -> ChunkingResult:
        ...
        nst = set(get_category_policy(self.subject).never_split_types)
        return segment_document_with_stats(document, never_split_types=nst)

    def enrich(self, chunks: list[Chunk]) -> list[Chunk]:
        from app.platform.text_quality import cyrillic_garble_token_ratio

        for chunk in chunks:
            if cyrillic_garble_token_ratio(chunk.content) > 0.30:
                if not chunk.metadata:
                    chunk.metadata = {}
                chunk.metadata["quarantined"] = True

        # Future seam for formula/figure/table enrichment.
        return list(chunks)
...
class MathPipeline(BaseSubjectPipeline):
    ...
    def parse(self, source: ManifestSource) -> ParsedDocument:
        if source.parser.json_artifact_path is not None:
            ...
            if path.exists():
                from app.ingestion.infrastructure.pages_json import (
                    parsed_document_from_pages_json,
                )
...
class CategoryPipeline(BaseSubjectPipeline):
