...
def _recognize_image(processor, model, device, dtype, png_path: Path) -> str:
    ...
    with torch.no_grad():
        # Greedy decoding on small/sparse crops (e.g. a formula-only region) can
        # collapse into a degenerate repetition loop that burns the whole
        # max_new_tokens budget on junk tokens (observed empirically, GRO-222 /
        # S8b spike: two chemistry-formula pages produced thousands of repeated
        # tokens with zero real content). A no-repeat-ngram guard plus a mild
        # repetition penalty breaks the loop without changing the model.
