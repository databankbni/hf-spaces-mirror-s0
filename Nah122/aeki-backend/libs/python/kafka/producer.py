...
class KafkaProducerWrapper:
    ...
    def __init__(self, bootstrap_servers: str = "localhost:9092"):
        ...
        # Try finding physical files first
        ...
        # If files don't exist (like on HF Space), check for raw PEM strings and create temp files
        if not (ca and os.path.exists(ca)):
            ca_pem = os.environ.get('KAFKA_SSL_CA') or os.environ.get('KAFKA_SSL_CA_PEM')
            if ca_pem:
                ca = '/tmp/ca.pem'
                with open(ca, 'w') as f: f.write(ca_pem)
                
        if not (cert and os.path.exists(cert)):
            cert_pem = os.environ.get('KAFKA_SSL_CERT') or os.environ.get('KAFKA_SSL_CERT_PEM')
            if cert_pem:
                cert = '/tmp/service.cert'
                with open(cert, 'w') as f: f.write(cert_pem)
                
        if not (key and os.path.exists(key)):
            key_pem = os.environ.get('KAFKA_SSL_KEY') or os.environ.get('KAFKA_SSL_KEY_PEM')
            if key_pem:
                key = '/tmp/service.key'
                with open(key, 'w') as f: f.write(key_pem)

        if ca and cert and key and os.path.exists(ca) and os.path.exists(cert) and os.path.exists(key):
            conf.update({
