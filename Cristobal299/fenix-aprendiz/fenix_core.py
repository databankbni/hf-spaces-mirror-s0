...
PERSONALIDAD_BASE = (
    ...
    "La verdad tiene un solo camino: primero busca en la base de datos, y solo "
    "después contesta apoyándote en lo que encontraste ahí. Si el bloque de "
    "'Fragmentos relevantes' está vacío o no aparece más abajo, significa que "
    "la búsqueda en la base de datos no encontró nada relacionado con la "
    "pregunta: en ese caso dilo explícitamente ('no encontré esto en tus "
    "libros indexados') en vez de responder con reflexiones espirituales "
    "genéricas inventadas. Nunca reemplaces la ausencia de datos reales con "
    "poesía mística improvisada.\n\n"
...
def cargar_contexto_pdf(pregunta: str, limite: int = 10):
    ...
    try:
        ...
        # FIX VERDAD: antes, si ningun candidato tenia coincidencia real de
        # palabras clave (puntuar(c) == 0 para todos), igual se devolvian
        # fragmentos al azar (candidatos[:limite]). Eso hacia que Fenix
        # recibiera "contexto" sin relacion real con la pregunta y, al decir
        # el prompt "prioriza esta informacion", el modelo mezclaba ese ruido
        # con su propio estilo, produciendo respuestas misticas genericas no
        # basadas en el libro real. Ahora, sin coincidencia real, se devuelve
        # vacio: Fenix debe admitir que no encontro nada en la base de datos.
        return mejores
    except Exception as e:
