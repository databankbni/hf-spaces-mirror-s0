...
CANONICALIZATION_VERSION = "v192_16"
RELATIONSHIP_TABLE_VERSION = "v192_16"
...
def infer_energy_from_text(*values: Any) -> str:
    ...
    if any(token in body for token in ("双擎", "油电混动", "hev", "2.5hg", "2.5hs")):
        return "HEV"
    # BMW combustion variants such as 735Li/740Li have no literal fuel token
    # in the trim, yet must not remain UNKNOWN and lose their C2B/B2C bridge.
    if re.search(r"(?<!\d)(?:730|735|740|745|750|760)(?:li|i|d|e)?", body):
        return "ICE"
    ...
    *,
    ...
    if strong != UNKNOWN and meta["energy_type"] != strong:
        ...
        return {
            **meta,
            "energy_type": strong,
            "energy_normalization_source": "V192_16_TEXT_OVERRIDES_RAW_FIELD",
...
def _parse_grade(norm: str) -> str:
    ...
    matched = [label for label, tokens in grade_patterns if any(token in norm for token in tokens)]
    if matched:
        # M sport/S-line describe a package; they do not replace the underlying
        # leading/luxury/comfort grade. Keeping both prevents distinct trims from
        # collapsing into one canonical key.
        return "+".join(matched)
    ...
    *,
    ...
    power_code = _first(
        [
            ...
            # BMW 7-series power variants must never collapse into one
            # long-wheelbase/M-sport semantic key (for example 735Li vs 740Li).
            r"(?<!\d)(730|735|740|745|750|760)(li|i|d|e)?",
            r"(?<!\d)(320|325|330|318|520|525|530|535)(li|i|d|e)?(?!\d)",
            r"c\s*(180|200|260|300)\s*(l)?(?![a-z0-9])",
            r"(?<!\d)(652|653|625)\s*t(?![a-z0-9])",
            r"(?<!\d)(35|40|45|50|55)\s*(tfsi|tdi)(?![a-z0-9])",
            r"(?<!\d)(180|200|280|330|380|530)\s*(tsi|tdi)(?![a-z0-9])",
            r"(?<![a-z0-9])(2\.0gvp|2\.0hg|2\.0hxs|2\.0hs|2\.0g)(?![a-z0-9])",
    ...
    if not power_code:
        if "coopers" in norm:
            ...
            power_code = "cooper"
        elif "cabrio" in norm or "敞篷" in norm:
            power_code = "cabrio"
    ...
    if not body_type:
        if "陆上公务舱" in norm:
            body_type = "land_business"
        elif "陆尊" in norm:
            body_type = "es_luzun"
        elif "陆尚" in norm:
            body_type = "es_lushang"
    ...
    # This is the strict exact-trim key used by point-price retrieval.  A
    # semantic signature alone is deliberately insufficient: two trims can
    # share powertrain, wheelbase and package labels while still differing in
    # equipment and price.  Alias handling happens before this point through
    # prefix/year/punctuation normalization; the normalized trim is then kept
    # as a final discriminator for every vehicle, not only sparse records.
    if norm:
        fields.append(f"trim={norm}")
...
def add_v192_16_semantic_columns(frame: pd.DataFrame) -> pd.DataFrame:
    result = frame.copy()
    ...
    result["condition_source_v192_16"] = condition["condition_source_v192_13"]
    result["condition_risk_level_v192_16"] = condition["condition_risk_level_v192_13"]
    return result
