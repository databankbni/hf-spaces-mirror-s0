"""Append-only ingestion of confirmed internal B2C sold records.

This is deliberately separate from ``v194_daily_actuals`` because a realised
retail sale is not a C2B purchase target.  It can become market context or a
B2C-to-C2B bridge input only after the sale is confirmed and ingested.
"""
...
from .v194_daily_actuals import business_event_dedup_key
from .v194_price_policy import build_evidence_warehouse


DAILY_B2C_FILENAME = "daily_confirmed_b2c_sold_actuals.parquet"

B2C_ALIASES: dict[str, tuple[str, ...]] = {
    "price": ("最新订单成交价", "b2c_sold_price_yuan", "售车成交价", "销售成交价", "成交价", "price"),
    "event_time": ("已售时间", "sold_time", "订单成交时间", "成交时间", "event_time", "target_date"),
    ...
    "order_id": ("order_id", "订单ID", "销售订单ID", "车源商品ID"),
}


def _column(frame: pd.DataFrame, field: str) -> pd.Series:
    for column in B2C_ALIASES[field]:
...
def _source_row_hash(frame: pd.DataFrame) -> pd.Series:
    ...
    return material.map(lambda text: hashlib.sha256(f"b2c|{text}".encode("utf-8")).hexdigest()[:24])


def normalize_confirmed_b2c_actuals(
    source: pd.DataFrame,
    *,
    ingested_at: Any,
    source_name: str = "daily_confirmed_internal_b2c_sold",
    price_unit: str = "auto",
) -> pd.DataFrame:
    """Convert a confirmed B2C sold file into a quality-gated evidence table."""
    ...
    result["source_type"] = "internal_b2c_sold"
    result["price_type"] = "b2c_sold_actual"
    ...
    result["observation_id"] = [f"daily_b2c_{source_name}_{i}" for i in result.index]

    normalized = build_evidence_warehouse(result)
    normalized["pricing_available_at"] = ingested
    normalized["runtime_candidate_lifecycle_key"] = np.where(
        normalized["vehicle_id_hash"].fillna("").astype(str).str.strip().ne(""),
        "VEHICLE|"
        + normalized["vehicle_id_hash"].astype(str)
        + "|B2C|"
        + normalized["event_time"].astype(str)
        + "|"
        + normalized["price_yuan"].astype(str),
        "CLUE|"
        + normalized["clue_id_hash"].astype(str)
        + "|B2C|"
        + normalized["event_time"].astype(str)
        + "|"
        + normalized["price_yuan"].astype(str),
    ...
    normalized["daily_business_event_key"] = business_event_dedup_key(normalized, "B2C")
    normalized["daily_actual_ingestion_status"] = np.where(
        normalized["allowed_for_b2c_market_reference"].fillna(False)
        | normalized["allowed_for_c2b_bridge_input"].fillna(False),
        "ELIGIBLE_FOR_FUTURE_B2C_REFERENCE",
        "RETAINED_AUDIT_NOT_ELIGIBLE",
    )
    return normalized


def append_confirmed_b2c_actuals(
    source: pd.DataFrame,
    *,
    root: Path,
    ingested_at: Any,
    ...
    """Append new B2C facts, returning (new_rows, full_daily_store)."""
    target = root / "data" / "v194" / DAILY_B2C_FILENAME
    target.parent.mkdir(parents=True, exist_ok=True)
    new_rows = normalize_confirmed_b2c_actuals(
    ...
    combined["daily_business_event_key"] = business_event_dedup_key(combined, "B2C")
