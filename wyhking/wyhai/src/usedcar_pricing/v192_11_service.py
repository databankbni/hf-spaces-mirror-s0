...
from .v192_12_semantics import canonicalize_trim, normalize_energy_type


PRICING_ENGINE_VERSION = "192.11.0"
MODEL_VERSION = "v192.8_historical_market_core"
POLICY_VERSION = "v192.11_real_runtime_package_policy"
EVIDENCE_CARD_VERSION = "v192.11_evidence_card"
...
class V19211QuoteStateStore:
    ...
    def _initialize(self) -> None:
        with self._connect() as connection:
            connection.execute(
                """
                CREATE TABLE IF NOT EXISTS quote_state_v192_11 (
            ...
            connection.execute(
                "CREATE INDEX IF NOT EXISTS qsv11_vehicle_idx "
                "ON quote_state_v192_11(vehicle_id, created_at)"
            )
            connection.execute(
                "CREATE INDEX IF NOT EXISTS qsv11_lifecycle_idx "
                "ON quote_state_v192_11(lifecycle_id, created_at)"
            )
            connection.execute(
                "CREATE INDEX IF NOT EXISTS qsv11_fingerprint_idx "
                "ON quote_state_v192_11(stable_vehicle_fingerprint, created_at)"
            )
    ...
    def save(self, state: dict[str, Any]) -> None:
        with self._lock, self._connect() as connection:
            connection.execute(
                """
                INSERT OR REPLACE INTO quote_state_v192_11 (
    ...
    def load_by_quote_id(self, quote_id: str) -> dict[str, Any] | None:
        ...
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM quote_state_v192_11 "
    ...
    def load_latest_by_vehicle_id(
        ...
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM quote_state_v192_11 WHERE vehicle_id = ? "
    ...
    def load_latest_by_lifecycle_id(
        ...
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM quote_state_v192_11 WHERE lifecycle_id = ? "
                "ORDER BY created_at DESC LIMIT 1",
                (lifecycle_id,),
            ).fetchone()
        return _decode_row(row)
...
@dataclass
class V19211PricingService:
    engine: PricingEngine
    state_store: V19211QuoteStateStore
    ...
    def _apply_guard_and_schema(
        ...
        *,
        ...
        guard = V1927ServingQuoteService(
            entrypoint_name="V19211PricingService.quote"
        ).quote(
            raw_price=raw_price,
...
def normalize_api_payload(
    ...
    energy_meta = normalize_energy_type(
        ...
        or result.get("is_new_energy"),
        brand=result.get("brand"),
        series=result.get("series"),
        trim=trim,
        is_new_energy=result.get("is_new_energy"),
    )
    energy = energy_meta["energy_type"]
    if energy == "UNKNOWN":
        energy = infer_energy_type(
            result.get("brand"),
            result.get("series"),
            model_year,
            trim,
    ...
    result["canonical_trim_key"] = canonical["canonical_trim_key"]
    result["normalized_trim"] = canonical["normalized_trim"]
    result["canonicalization_reason"] = canonical["canonicalization_reason"]
    result["canonicalization_confidence"] = canonical[
    ...
    meta = {
        ...
        "energy_mapping_source": (
            ...
            else energy_meta["energy_normalization_source"]
        ),
        ...
        "canonicalization_confidence": canonical[
            "canonicalization_confidence"
        ],
...
def _evidence_card(
    *,
    ...
    summary = (
        "v192.11 使用真实历史候选召回、时间过滤、语义TopK、统计基线、"
