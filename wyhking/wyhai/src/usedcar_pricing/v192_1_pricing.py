...
def query_statistical_prices(
    ...
    *,
    ...
    eligible = group.copy()
    hard_conflict = pd.Series(False, index=eligible.index)
    for column in ("same_brand", "same_series"):
        if column in eligible:
            hard_conflict |= eligible[column].ne(1)
    if "energy_conflict_flag" in eligible:
        hard_conflict |= eligible["energy_conflict_flag"].eq(1)
    elif "energy_match" in eligible:
        hard_conflict |= eligible["energy_match"].ne(1)
    if "semantic_invalid_reason" in eligible:
        hard_conflict |= eligible["semantic_invalid_reason"].isin(
            [
                "brand_conflict",
                "series_conflict",
    ...
    eligible = eligible[~hard_conflict]
    if "invalid_candidate_flag" in eligible:
        eligible = eligible[eligible["invalid_candidate_flag"].ne(1)]
    top = (
        eligible.sort_values("ranker_score", ascending=False, kind="stable")
        .head(top_k)
        .copy()
    )
    if top.empty:
        ...
        result = {
            ...
            "candidate_price_p25": np.nan,
            "candidate_price_p50": np.nan,
            "candidate_price_p75": np.nan,
            "candidate_dispersion": np.nan,
            "latest_candidate_days": np.nan,
            "best_retrieval_level": "",
            "best_retrieval_level_index": 99,
            ...
            "weighted_p40": np.nan,
            "trimmed_mean": np.nan,
            "local_regression": np.nan,
            "local_regression_trace": "",
            "top_k": top_k,
        }
        return result, top
    ...
    result = {
        ...
        "weighted_p25": weighted_quantile(prices, weights, 0.25),
        "weighted_p30": weighted_quantile(prices, weights, 0.30),
...
def build_statistical_table(frame: pd.DataFrame, top_k_values: tuple[int, ...] = (5, 10, 15)) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for query_id, group in frame.groupby("query_id", sort=False):
        ...
        for top_k in top_k_values:
            ...
            if base is None:
                base = {key: value for key, value in result.items() if key not in {"weighted_median", "weighted_p25", "weighted_p30", "weighted_p40", "trimmed_mean", "local_regression", "top_k"}}
            for method in ("weighted_median", "weighted_p25", "weighted_p30", "weighted_p40", "trimmed_mean"):
    ...
    return pd.DataFrame(rows.values()), weights
