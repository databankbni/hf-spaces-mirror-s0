...
def load_data(prepared_dir: Path) -> pd.DataFrame:
    csv_path = prepared_dir / "data/processed/pricing_training_wide_enriched_combined.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"未找到两源宽表: {csv_path}")
...
def phase0_report(df: pd.DataFrame, output_root: Path) -> None:
    ...
    (report_dir / "PHASE0_DATA_AND_FEATURE_CHECK.md").write_text("\n".join(text), encoding="utf-8")
...
def add_rag(train: pd.DataFrame, valid: pd.DataFrame, test: pd.DataFrame, task: str, output_root: Path, tag: str):
    target = TARGET_BY_TASK[task]
    cache_dir = ensure_dir(output_root / "artifacts/rag_features_two_source")
...
def run_phase1(splits: dict, output_root: Path, args) -> None:
    ...
    pd.DataFrame(skipped).to_csv(model_result_dir / "phase1_skipped_models.csv", index=False, encoding="utf-8-sig")
    phase1_specs = [s for s in candidate_specs() if s.phase1 and not s.skipped_reason]
    for task in TASKS:
        result_path = model_result_dir / f"phase1_screening_{task}.csv"
        target = TARGET_BY_TASK[task]
    ...
    for task in TASKS:
        path = model_result_dir / f"phase1_screening_{task}.csv"
    ...
    (reports / "PHASE1_MODEL_SCREENING_REPORT.md").write_text("\n".join(lines), encoding="utf-8")


def select_phase2_models(output_root: Path, task: str, top_k: int = 6) -> list[str]:
    path = output_root / f"artifacts/model_results/phase1_screening_{task}.csv"
    df = pd.read_csv(path)
...
def run_phase2(splits: dict, output_root: Path, args) -> None:
    model_result_dir = ensure_dir(output_root / "artifacts/model_results")
    for task in TASKS:
        result_path = model_result_dir / f"model_comparison_{task}_full.csv"
        train, valid, test = splits[task]
    ...
    for task in TASKS:
        path = model_result_dir / f"model_comparison_{task}_full.csv"
        df = pd.read_csv(path) if path.exists() else pd.DataFrame()
    ...
    (output_root / "reports/PHASE2_FULL_TRAINING_REPORT.md").write_text("\n".join(lines), encoding="utf-8")


def best_full(output_root: Path, task: str) -> pd.Series:
    path = output_root / f"artifacts/model_results/model_comparison_{task}_full.csv"
    df = pd.read_csv(path)
...
def run_phase3(df: pd.DataFrame, splits: dict, output_root: Path, args) -> None:
    ...
    (output_root / "reports/PHASE3_SPECIAL_EXPERIMENTS_REPORT.md").write_text("\n".join(lines), encoding="utf-8")
...
def write_final_reports(output_root: Path) -> None:
    ...
    best_rows = []
    for task in TASKS:
        full_path = output_root / f"artifacts/model_results/model_comparison_{task}_full.csv"
    ...
    (output_root / "reports/FINAL_MODEL_SELECTION_REPORT.md").write_text("\n".join(lines), encoding="utf-8")
    (output_root / "reports/HUMAN_REVIEW_EVALUATION_REPORT.md").write_text("\n".join([
        "# Human Review 评估报告",
        "",
    ...
    (output_root / "reports/NEXT_DATA_IMPROVEMENT_PLAN_TWO_SOURCE.md").write_text("\n".join([
        "# 下一步数据补强计划",
        "",
...
def main():
    ...
    parser.add_argument("--prepared-dir", default="/Users/bytedance/Downloads/used_car_pricing_two_sources_prepared")
