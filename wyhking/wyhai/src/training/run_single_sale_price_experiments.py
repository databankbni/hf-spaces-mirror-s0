...
def load_data(prepared_dir: Path) -> pd.DataFrame:
    csv_path = prepared_dir / "data/processed/pricing_model_training_single_sale_price.csv"
...
def phase0_report(df: pd.DataFrame, output_root: Path) -> None:
    ...
    (report_dir / "PHASE0_DATA_AND_FEATURE_CHECK_SINGLE_SALE_PRICE.md").write_text("\n".join(text), encoding="utf-8")
...
def add_rag(train: pd.DataFrame, valid: pd.DataFrame, test: pd.DataFrame, task: str, output_root: Path, tag: str):
    target = TARGET_BY_TASK[task]
    cache_dir = ensure_dir(output_root / "artifacts/rag_features_single_sale_price")
...
def run_phase1(splits: dict, output_root: Path, args) -> None:
    ...
    pd.DataFrame(skipped).to_csv(model_result_dir / "phase1_skipped_models_single_sale_price.csv", index=False, encoding="utf-8-sig")
    phase1_specs = [s for s in candidate_specs() if s.phase1 and not s.skipped_reason]
    for task in TASKS:
        result_path = model_result_dir / f"phase1_screening_{task}_single_sale_price.csv"
        target = TARGET_BY_TASK[task]
    ...
    for task in TASKS:
        path = model_result_dir / f"phase1_screening_{task}_single_sale_price.csv"
    ...
    (reports / "PHASE1_MODEL_SCREENING_REPORT_SINGLE_SALE_PRICE.md").write_text("\n".join(lines), encoding="utf-8")


def select_phase2_models(output_root: Path, task: str, top_k: int = 6) -> list[str]:
    path = output_root / f"artifacts/model_results/phase1_screening_{task}_single_sale_price.csv"
    df = pd.read_csv(path)
    ok = df[df["status"] == "completed"].copy()
    base = ["LightGBM_L2", "LightGBM_L1", "XGBoost_squarederror", "CatBoost_RMSE", "Sklearn_HistGBR"]
...
def run_phase2(splits: dict, output_root: Path, args) -> None:
    model_result_dir = ensure_dir(output_root / "artifacts/model_results")
    for task in TASKS:
        result_path = model_result_dir / f"model_comparison_{task}_single_sale_price.csv"
        train, valid, test = splits[task]
    ...
    for task in TASKS:
        path = model_result_dir / f"model_comparison_{task}_single_sale_price.csv"
        df = pd.read_csv(path) if path.exists() else pd.DataFrame()
    ...
    (output_root / "reports/MODEL_TRAINING_REPORT_SINGLE_SALE_PRICE.md").write_text("\n".join(lines), encoding="utf-8")


def best_full(output_root: Path, task: str) -> pd.Series:
    path = output_root / f"artifacts/model_results/model_comparison_{task}_single_sale_price.csv"
    df = pd.read_csv(path)
...
def run_phase3(df: pd.DataFrame, splits: dict, output_root: Path, args) -> None:
    ...
    (output_root / "reports/PHASE3_SPECIAL_EXPERIMENTS_REPORT_SINGLE_SALE_PRICE.md").write_text("\n".join(lines), encoding="utf-8")
...
def write_final_reports(output_root: Path) -> None:
    ...
    best_rows = []
    for task in TASKS:
        full_path = output_root / f"artifacts/model_results/model_comparison_{task}_single_sale_price.csv"
    ...
    (output_root / "reports/B2C_SINGLE_SALE_PRICE_FIX_REPORT.md").write_text("\n".join(lines), encoding="utf-8")
    ...
    (output_root / "reports/NEXT_DATA_IMPROVEMENT_PLAN_SINGLE_SALE_PRICE.md").write_text("\n".join([
        "# 下一步数据补强计划",
        "",
...
def main():
    ...
    parser.add_argument("--prepared-dir", default="/Users/bytedance/Downloads/used_car_pricing_single_sale_price_v3")
