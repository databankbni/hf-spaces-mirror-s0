...
from usedcar_pricing.v192_15_semantics import (
...
PRICING_ENGINE_VERSION = "192.15.0"
MODEL_VERSION = "v192_15_semantic_runtime_existing_residual"
POLICY_VERSION = "v192_15_full_semantic_runtime_policy"
EVIDENCE_CARD_VERSION = "v192_15_evidence_card"
RETRIEVAL_POLICY_VERSION = "v192_15_semantic_relationship_policy_v1"
BUILD_TIME = datetime.now(timezone.utc).isoformat()

_ENGINE: "HistoricalV19215PricingEngine | None" = None
_ENGINE_LOCK = threading.Lock()
_READY_CACHE: dict[str, Any] | None = None


V19215_OBSERVATION_RUNTIME_COLUMNS = [
    *OBSERVATION_RUNTIME_COLUMNS,
    "raw_trim",
    ...
    "condition_source_v192_15",
    "store_name",
]


def _load_v19215_observations(root: Path) -> pd.DataFrame:
    path = root / "data/v192_15/vehicle_source_price_observation_v192_15_semantic.parquet"
    if not path.exists():
        raise FileNotFoundError(path)
    frame = _read_parquet_columns(path, V19215_OBSERVATION_RUNTIME_COLUMNS)
    ...
    frame["trim_key_original_v192_15"] = frame["trim_key"].fillna("").astype(str)
...
def _dedupe_relationships_for_runtime(relationships: pd.DataFrame) -> pd.DataFrame:
    ...
    result["relationship_type_v192_15_original"] = result["relationship_type"]
...
def enrich_candidates_v19215(
    ...
    """Runtime enrichment that gives v192.15 canonical relations priority.

    The older v192.7 pipeline joins relationship rows by raw trim strings.
    v192.15 deliberately treats canonical_trim_key as the semantic identity,
    ...
    rel = rel.rename(
        columns={
            ...
            **{col: f"{col}__v19215" for col in RELATIONSHIP_OVERRIDE_COLUMNS},
        }
    )
    ...
    override_mask = enriched["relationship_type__v19215"].notna()
    for col in RELATIONSHIP_OVERRIDE_COLUMNS:
        v14_col = f"{col}__v19215"
    ...
    enriched["v192_15_relationship_override_applied"] = override_mask.astype(int)
...
def normalized_query_v19215(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "query_id": str(payload.get("request_id") or payload.get("quote_id") or "v192_15_live_query"),
        ...
        "condition_source_v192_15": "USER_PROVIDED"
...
class HistoricalV19215PricingEngine(HistoricalV1928PricingEngine):
    def __init__(self, root: Path = ROOT) -> None:
        ...
        self.root = root
        observations = _load_v19215_observations(root)
        observations["trim_key_original_v192_15"] = observations["trim_key"].fillna("").astype(str)
        ...
        self.relationships = _read_parquet_columns(
            root / "data/v192_15/trim_relationship_table.parquet",
            RELATIONSHIP_RUNTIME_COLUMNS,
        )
        ...
        self.enrich_candidates = lambda candidates, quality, relationships, query_energy: enrich_candidates_v19215(
            candidates,
            quality,
            relationships,
            query_energy,
    ...
    def quote(self, payload: dict[str, Any]) -> dict[str, Any]:
        query = normalized_query_v19215(payload)
        ...
        selected["v192_15_point_baseline_allowed"] = (
        ...
        if query.get("condition_source_v192_15") == "SYSTEM_DEFAULT_GOOD_CONDITION":
...
def get_engine() -> HistoricalV19215PricingEngine:
    global _ENGINE
    with _ENGINE_LOCK:
        if _ENGINE is None:
            _ENGINE = HistoricalV19215PricingEngine(ROOT)
    return _ENGINE


def _patch_v19215_result(result: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    normalized = normalized_query_v19215(payload)
    result["pricing_engine_used"] = "V192_15"
    ...
    result["modelName"] = "v192.15-full-semantic-runtime"
    result["reason"] = "v192.15 使用全车型语义Canonical Key、能源/车况标准化、严格关系表和真实历史候选生成报价证据。"
    result["input_normalization"] = {
        ...
        "normalized_query": {
            ...
            "condition_source": normalized["condition_source_v192_15"],
        },
    ...
    card.update(
        {
            ...
            "summary": "v192.15按统一款型语义、能源识别和候选关系表生成，可从证据卡复核候选、权重和调整。",
...
def minimal_real_payload() -> dict[str, Any]:
    return {
        "request_id": "v192_15_ready_probe",
        "brand": "宝马",
        "series": "宝马3系",
        ...
        "vehicle_id": "v19215-ready-probe",
        "lifecycle_id": "v19215-ready-probe-life",
    }


def quote_with_v19215_service(
    payload: dict[str, Any],
    legacy_pricing_callable: Callable[[dict[str, Any]], dict[str, Any]],
) -> dict[str, Any]:
    store_path = Path(
        payload.get("_quote_state_store_path")
        or os.environ.get("V19215_QUOTE_STATE_DB", "")
        or ROOT / "data/runtime/v192_15_quote_state.sqlite"
    )
    ...
    if raw_engine and raw_engine != "V192_15":
        result["pricing_engine_used"] = raw_engine
        result["fallback_reason"] = result.get("fallback_reason") or "V192_15_ENGINE_FAILED"
        return result
    return _patch_v19215_result(result, payload)
...
def _required_files() -> list[Path]:
    return [
        ...
        ROOT / "results/v192_2/v192_2_observation_quality.parquet",
        ROOT / "data/v192_15/trim_relationship_table.parquet",
        ROOT / "data/v192_15/canonical_trim_table.parquet",
        ROOT / "results/audit/v192_2_series_calibration_oof_audit.csv",
        ROOT / "models/v192_2/v192_2_base_residual_model.joblib",
    ]


def v19215_readiness_check(force: bool = False) -> dict[str, Any]:
    global _READY_CACHE
    if _READY_CACHE is not None and not force:
        return dict(_READY_CACHE)
    checks: dict[str, Any] = {
        "pricing_engine_version": PRICING_ENGINE_VERSION,
        "entrypoint_uses_v192_15": True,
    ...
    try:
        store = V19211QuoteStateStore(ROOT / "data/runtime/v192_15_ready_check.sqlite")
        ...
        result = _patch_v19215_result(service.quote(minimal_real_payload()), minimal_real_payload())
        price_result = result.get("price_result") or {}
        checks["real_prediction_succeeded"] = bool(
            price_result.get("final_price") is not None
            and result.get("pricing_engine_used") == "V192_15"
        )
    ...
    checks["ready"] = (
        ...
        and checks["entrypoint_uses_v192_15"]
        and checks["engine_loaded"]
        and checks["real_prediction_succeeded"]
        and checks["real_prediction_pricing_engine_used"] == "V192_15"
