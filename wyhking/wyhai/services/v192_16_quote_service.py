...
from usedcar_pricing.v192_16_semantics import (
...
PRICING_ENGINE_VERSION = "192.16.0"
MODEL_VERSION = "v192_16_semantic_runtime_residual_disabled"
POLICY_VERSION = "v192_16_confidence_calibrated_runtime_policy"
EVIDENCE_CARD_VERSION = "v192_16_evidence_card"
RETRIEVAL_POLICY_VERSION = "v192_16_semantic_relationship_policy_v1"
CONFIDENCE_POLICY_VERSION = "v192_16_latest30_calibrated_conservative"
RESIDUAL_POLICY_VERSION = "v192_16_disabled_pending_retrain"
BUILD_TIME = datetime.now(timezone.utc).isoformat()

_ENGINE: "HistoricalV19216PricingEngine | None" = None
_ENGINE_LOCK = threading.Lock()
_READY_CACHE: dict[str, Any] | None = None


V19216_OBSERVATION_RUNTIME_COLUMNS = [
    *OBSERVATION_RUNTIME_COLUMNS,
    "raw_trim",
    ...
    "condition_source_v192_16",
    "store_name",
]


def _load_v19216_observations(root: Path) -> pd.DataFrame:
    path = root / "data/v192_16/vehicle_source_price_observation_v192_16_semantic.parquet"
    if not path.exists():
        raise FileNotFoundError(path)
    frame = _read_parquet_columns(path, V19216_OBSERVATION_RUNTIME_COLUMNS)
    ...
    frame["trim_key_original_v192_16"] = frame["trim_key"].fillna("").astype(str)
...
def _dedupe_relationships_for_runtime(relationships: pd.DataFrame) -> pd.DataFrame:
    ...
    result["relationship_type_v192_16_original"] = result["relationship_type"]
...
def enrich_candidates_v19216(
    ...
    """Runtime enrichment that gives v192.16 canonical relations priority.

    The older v192.7 pipeline joins relationship rows by raw trim strings.
    v192.16 deliberately treats canonical_trim_key as the semantic identity,
    ...
    rel = rel.rename(
        columns={
            ...
            **{col: f"{col}__v19216" for col in RELATIONSHIP_OVERRIDE_COLUMNS},
        }
    )
    ...
    override_mask = enriched["relationship_type__v19216"].notna()
    for col in RELATIONSHIP_OVERRIDE_COLUMNS:
        v14_col = f"{col}__v19216"
    ...
    enriched["v192_16_relationship_override_applied"] = override_mask.astype(int)
...
def normalized_query_v19216(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "query_id": str(payload.get("request_id") or payload.get("quote_id") or "v192_16_live_query"),
        ...
        "condition_source_v192_16": "USER_PROVIDED"
...
def _v19216_confidence_policy(row: dict[str, Any], query: dict[str, Any]) -> tuple[str, list[str]]:
    reasons: list[str] = []
    baseline_count = float(row.get("baseline_candidate_count_v192_12") or row.get("baseline_candidate_count") or 0)
    same_trim = float(row.get("same_trim_candidate_count") or 0)
    t4_weight = float(row.get("t4_fallback_weight") or 0)
    same_city = float(row.get("same_city_weight") or 0)
    recent_90d = float(row.get("evidence_weight_within_90d") or 0)
    interval_low = float(row.get("business_interval_low") or 0)
    interval_high = float(row.get("business_interval_high") or 0)
    final_price = float(row.get("raw_price_before_guard") or row.get("statistical_baseline_price") or 0)
    interval_width = (
        (interval_high - interval_low) / final_price
        if final_price > 0 and interval_high > interval_low
        else 9.0
    )
    if query.get("energy_field_conflict_flag"):
        reasons.append("ENERGY_FIELD_CONFLICT_DOWNGRADED")
    if t4_weight > 0:
        reasons.append("T4_PRESENT_NOT_HIGH_OR_MEDIUM")
    if same_city <= 0:
        reasons.append("NO_SAME_CITY_EVIDENCE")
    if recent_90d <= 0:
        reasons.append("RECENT_90D_EVIDENCE_MISSING")
    if interval_width > 0.35:
        reasons.append("INTERVAL_WIDTH_TOO_WIDE")
    if same_trim >= 1 and baseline_count <= 2 and t4_weight <= 0 and not query.get("energy_field_conflict_flag"):
        # latest30 calibration supports this small, strict bucket as Medium.
        confidence = "MEDIUM"
        reasons.append("MEDIUM_BY_EXACT_LOCAL_BUCKET_CALIBRATION")
    elif baseline_count > 0:
        confidence = "LOW"
        reasons.append("LOW_BY_EVIDENCE_NOT_CALIBRATED_FOR_AUTO")
    else:
        confidence = "MANUAL"
        reasons.append("MANUAL_NO_STRICT_BASELINE")
    # HIGH is deliberately disabled until rolling calibration proves it.
    reasons.append("HIGH_DISABLED_PENDING_TRUSTED_SEGMENT")
    if query.get("condition_source_v192_16") == "SYSTEM_DEFAULT_GOOD_CONDITION":
        reasons.append("SYSTEM_DEFAULT_GOOD_CONDITION")
    return confidence, list(dict.fromkeys(reasons))


class HistoricalV19216PricingEngine(HistoricalV1928PricingEngine):
    def __init__(self, root: Path = ROOT) -> None:
        ...
        self.root = root
        observations = _load_v19216_observations(root)
        observations["trim_key_original_v192_16"] = observations["trim_key"].fillna("").astype(str)
        ...
        self.relationships = _read_parquet_columns(
            root / "data/v192_16/trim_relationship_table.parquet",
            RELATIONSHIP_RUNTIME_COLUMNS,
        )
        ...
        self.enrich_candidates = lambda candidates, quality, relationships, query_energy: enrich_candidates_v19216(
            candidates,
            quality,
            relationships,
            query_energy,
    ...
    def quote(self, payload: dict[str, Any]) -> dict[str, Any]:
        query = normalized_query_v19216(payload)
        ...
        selected["v192_16_point_baseline_allowed"] = (
        ...
        if stats.empty:
            ...
            return {
                ...
                "price_trace": {
                    ...
                    "residual_policy": "DISABLED_PENDING_RETRAIN",
                    "residual_policy_version": RESIDUAL_POLICY_VERSION,
                    "residual_rejection_reason_codes": [
                        "NO_STRICT_COMPARABLE_BASELINE",
                        "RESIDUAL_DISABLED_PENDING_RETRAIN",
                    ],
                ...
                "confidence_policy_version": CONFIDENCE_POLICY_VERSION,
                "residual_policy_version": RESIDUAL_POLICY_VERSION,
                "confidence_downgrade_reasons": ["MANUAL_NO_STRICT_BASELINE"],
        ...
        row["baseline_candidate_count"] = int(
            selected["used_for_statistical_baseline_v192_12"].fillna(0).astype(float).gt(0).sum()
        )
        ...
        confidence, confidence_reasons = _v19216_confidence_policy(row, query)
        baseline_price = float(row["statistical_baseline_price"])
        raw_residual_ratio = float(residual_trace.get("raw_residual_ratio") or 0.0)
        raw_residual_amount = float(residual_trace.get("raw_residual_amount") or 0.0)
        trace = {
            "statistical_baseline_price": baseline_price,
            **residual_trace,
            "raw_residual_ratio": raw_residual_ratio,
            "raw_residual_amount": raw_residual_amount,
            "base_residual_adjustment_amount": 0.0,
            "low_price_adjustment_amount": 0.0,
            "series_calibration_adjustment_amount": 0.0,
            "raw_price_before_guard": baseline_price,
            "final_price_after_guard": baseline_price,
            "final_residual_ratio": 0.0,
            "final_residual_amount": 0.0,
            "residual_policy": "DISABLED_PENDING_RETRAIN",
            "residual_policy_version": RESIDUAL_POLICY_VERSION,
            "residual_rejection_reason_codes": [
                "RESIDUAL_DISABLED_PENDING_RETRAIN",
                "V192_15_ABC_IDENTICAL_NO_VALID_LIFT",
            ],
            "confidence_policy_version": CONFIDENCE_POLICY_VERSION,
            "confidence_downgrade_reasons": confidence_reasons,
            "canonicalization_version": CANONICALIZATION_VERSION,
...
def get_engine() -> HistoricalV19216PricingEngine:
    global _ENGINE
    with _ENGINE_LOCK:
        if _ENGINE is None:
            _ENGINE = HistoricalV19216PricingEngine(ROOT)
    return _ENGINE


def _patch_v19216_result(result: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    normalized = normalized_query_v19216(payload)
    result["pricing_engine_used"] = "V192_16"
    ...
    result["confidence_policy_version"] = CONFIDENCE_POLICY_VERSION
    result["residual_policy_version"] = RESIDUAL_POLICY_VERSION
    ...
    result["modelName"] = "v192.16-full-semantic-runtime"
    result["reason"] = "v192.16 使用全车型语义Canonical Key、能源/车况标准化、严格关系表和真实历史候选生成报价证据。"
    result["input_normalization"] = {
        ...
        "normalized_query": {
            ...
            "condition_source": normalized["condition_source_v192_16"],
        },
    ...
    card.update(
        {
            ...
            "confidence_policy_version": CONFIDENCE_POLICY_VERSION,
            "residual_policy_version": RESIDUAL_POLICY_VERSION,
            "summary": "v192.16按统一款型语义、能源识别和候选关系表生成，可从证据卡复核候选、权重和调整。",
...
def get_version_payload() -> dict[str, Any]:
    return {
        ...
        "relationship_table_version": RELATIONSHIP_TABLE_VERSION,
        "evidence_card_version": EVIDENCE_CARD_VERSION,
        "git_commit": os.environ.get("GIT_COMMIT") or "workspace-no-git-v192_16",
...
def minimal_real_payload() -> dict[str, Any]:
    return {
        "request_id": "v192_16_ready_probe",
        "brand": "宝马",
        "series": "宝马3系",
        ...
        "vehicle_id": "v19216-ready-probe",
        "lifecycle_id": "v19216-ready-probe-life",
    }


def quote_with_v19216_service(
    payload: dict[str, Any],
    legacy_pricing_callable: Callable[[dict[str, Any]], dict[str, Any]],
) -> dict[str, Any]:
    store_path = Path(
        payload.get("_quote_state_store_path")
        or os.environ.get("V19216_QUOTE_STATE_DB", "")
        or ROOT / "data/runtime/v192_16_quote_state.sqlite"
    )
    ...
    if raw_engine and raw_engine != "V192_16":
        result["pricing_engine_used"] = raw_engine
        result["fallback_reason"] = result.get("fallback_reason") or "V192_16_ENGINE_FAILED"
        return result
    return _patch_v19216_result(result, payload)
...
def _required_files() -> list[Path]:
    return [
        ...
        ROOT / "results/v192_2/v192_2_observation_quality.parquet",
        ROOT / "data/v192_16/trim_relationship_table.parquet",
        ROOT / "data/v192_16/canonical_trim_table.parquet",
        ROOT / "results/audit/v192_2_series_calibration_oof_audit.csv",
        ROOT / "models/v192_2/v192_2_base_residual_model.joblib",
    ]


def v19216_readiness_check(force: bool = False) -> dict[str, Any]:
    global _READY_CACHE
    if _READY_CACHE is not None and not force:
        return dict(_READY_CACHE)
    checks: dict[str, Any] = {
        "pricing_engine_version": PRICING_ENGINE_VERSION,
        "entrypoint_uses_v192_16": True,
    ...
    try:
        store = V19211QuoteStateStore(ROOT / "data/runtime/v192_16_ready_check.sqlite")
        ...
        result = _patch_v19216_result(service.quote(minimal_real_payload()), minimal_real_payload())
        price_result = result.get("price_result") or {}
        checks["real_prediction_succeeded"] = bool(
            price_result.get("final_price") is not None
            and result.get("pricing_engine_used") == "V192_16"
        )
    ...
    checks["ready"] = (
        ...
        and checks["entrypoint_uses_v192_16"]
        and checks["engine_loaded"]
        and checks["real_prediction_succeeded"]
        and checks["real_prediction_pricing_engine_used"] == "V192_16"
