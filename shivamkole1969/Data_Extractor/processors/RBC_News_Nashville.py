...
class RBCNewsNashvilleProcessor(BaseProcessor):
    PROCESSOR_NAME = "RBC News from Nashville"
    SUPPORTED_EXTENSIONS = [".pdf"]

    def process(self, filepath: str, job) -> str:
        job.message = "Initializing RBC News from Nashville extraction..."
        job.progress = 5
        ...
        job.message = "Extracting valuation tables..."
        job.progress = 10
        ...
        output_name = f"RBC_News_Nashville_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        output_path = self.output_folder / output_name

        columns = ordered_columns(rows)
        write_xlsx(output_path, rows, skipped_nr, audit, columns)

        job.output_file = output_name
        job.progress = 100
        job.message = "RBC News from Nashville extraction complete."
