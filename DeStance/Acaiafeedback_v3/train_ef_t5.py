...
MODEL_CHECKPOINT = "strozz1/t5_ef"
SAVE_DIR = "t5_ef_final_june"
...
for ex in raw:
    ...
    new_labels = [l for l in labels if not l.startswith("EP")]

    # opcional: descartar si se queda sin EF
    if any(l.startswith("EF") for l in new_labels):
        ex2 = dict(ex)
        ex2["labels"] = new_labels
        cleaned.append(ex2)
import re

def remove_ep_tags(text):
    return re.sub(r"<EP_[^>]*>", "", text)

targets = [remove_ep_tags(ex["sentence_with_labels"]) for ex in cleaned]
def clean_tags(text):
