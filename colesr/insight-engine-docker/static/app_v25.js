...
function drawCountryBoundaries(radius) {
  // Simplified continental outlines using verified geographic landmarks.
  // Each chain is a closed loop of ~8-12 real coastal points.
  // Great-circle interpolation smooths between them.
  const coastlines = [
    // === NORTH AMERICA ===
    // Point Barrow → Vancouver → SF → Baja → Yucatan → Miami → NYC → Newfoundland → Labrador → Baffin → back
    [ [71,-156],[60,-166],[55,-130],[49,-123],[37,-122],[25,-112],[16,-85],[21,-87],[25,-80],[36,-76],[41,-74],[47,-53],[55,-60],[66,-85],[71,-156] ],

    // === SOUTH AMERICA ===
    // Panama → Guyana → Recife → Rio → Uruguay → Cape Horn → Chile → Peru → Ecuador → Colombia → back
    [ [9,-77],[5,-52],[0,-46],[-8,-35],[-23,-43],[-35,-54],[-52,-69],[-40,-74],[-20,-70],[-5,-81],[2,-80],[9,-77] ],

    // === EUROPE ===
    // Gibraltar → Brittany → S England → Scotland → Norway → North Cape → NW Russia → Finland → Baltic → Poland → Black Sea → Turkey → Greece → Italy → Spain → back
    [ [36,-6],[43,-9],[48,-5],[51,1],[56,-4],[61,-2],[70,28],[68,40],[60,28],[54,14],[46,30],[41,29],[40,26],[38,12],[43,3],[36,-6] ],

    // === AFRICA ===
    // Morocco → W Sahara → Senegal → Guinea → Angola → Namibia → Cape of Good Hope → Durban → Mozambique → Somalia → Djibouti → Red Sea → Egypt → Libya → Tunisia → back
    [ [36,-6],[28,-12],[15,-17],[5,-12],[-5,12],[-15,12],[-35,20],[-30,32],[-15,40],[5,40],[12,44],[28,35],[32,33],[33,13],[37,10],[36,-6] ],

    // === ASIA ===
    // Bering → Kamchatka → Kurils → Japan → China → Vietnam → Singapore → Indonesia → India → Pakistan → Iran → Turkey → Russia → Siberia → back
    [ [66,-169],[53,160],[45,147],[35,140],[25,120],[10,107],[1,104],[-6,107],[8,77],[25,68],[25,57],[40,26],[45,50],[60,70],[66,130],[66,-169] ],

    // === AUSTRALIA ===
    // Darwin → Broome → Perth → Albany → Melbourne → Sydney → Brisbane → Cape York → back
    [ [-11,130],[-18,122],[-32,116],[-35,117],[-38,141],[-34,151],[-20,149],[-12,142],[-11,130] ],

    // === GREENLAND ===
    [ [84,-73],[82,-55],[78,-20],[75,-18],[71,-22],[68,-30],[60,-44],[60,-54],[60,-68],[67,-78],[76,-68],[80,-32],[84,-73] ],

    // === ANTARCTICA ===
    [ [-65,-180],[-65,-135],[-65,-90],[-65,-45],[-65,0],[-65,45],[-65,90],[-65,135],[-65,180] ],

    // === JAPAN ===
    [ [46,143],[43,146],[38,142],[35,140],[33,130],[31,131],[34,137],[39,142],[43,146],[46,143] ],
  ];

  // Bright cyan-blue, depthTest=true for proper occlusion
  const mat = new THREE.LineBasicMaterial({ color: 0x7dd3fc, transparent: true, opacity: 0.6, depthTest: true });
  ...
  coastlines.forEach(chain => {
    ...
    const seg = greatCirclePoints(chain[chain.length-1][0], chain[chain.length-1][1], chain[0][0], chain[0][1], radius, 10);
    allPts.push(...seg);
