...
function drawCountryBoundaries(radius) {
  ...
  // Linear interpolation between anchors, then projected to sphere surface.
  // 15 steps between anchors = smooth curves (~500 vectors per continent).
  const coastlines = [
  ...
  // Thicker, brighter boundary lines
  ...
  coastlines.forEach(chain => {
    ...
    for (let i = 0; i < chain.length - 1; i++) {
      const seg = interpolateCoastline(chain[i][0], chain[i][1], chain[i+1][0], chain[i+1][1], radius, 15);
      allPts.push(...seg);
    }
    // Close loop
    const seg = interpolateCoastline(chain[chain.length-1][0], chain[chain.length-1][1], chain[0][0], chain[0][1], radius, 15);
    allPts.push(...seg);
