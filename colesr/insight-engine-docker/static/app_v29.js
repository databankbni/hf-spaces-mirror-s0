...
function drawCountryBoundaries(radius) {
  ...
  // Coastline material: warm cyan for maximum contrast against dark blue globe/grid
  const mat = new THREE.LineBasicMaterial({ color: 0x60e5ff, transparent: true, opacity: 0.75, depthTest: true });
...
function initGlobe() {
  ...
  // Very subtle wireframe overlay — just for sphere shape, not grid
  const wireGeo = new THREE.SphereGeometry(R + 0.2, 24, 24);
