...
initTooltips();
// ============================================================================
// NEWS GLOBE v32 — Enhanced coastlines + Real-time sentiment + Alive effects
// ============================================================================
...
let globeSentimentPollTimer = null;
let globePulseTime = 0;
let globeRipples = [];
...
async function openNewsGlobe() {
  ...
  else { globeRenderer.setSize(window.innerWidth, window.innerHeight); startGlobeAnimation(); }
  await fetchGlobeData();
  startSentimentPolling();
  if (typeof lucide !== 'undefined') lucide.createIcons();
  if (!globeEscHandler) {
    globeEscHandler = (e) => { if (e.key === 'Escape') closeNewsGlobe(); };
...
function closeNewsGlobe() {
  ...
  if (globeEscHandler) { document.removeEventListener('keydown', globeEscHandler); globeEscHandler = null; }
}

function startSentimentPolling() {
  stopSentimentPolling();
  globeSentimentPollTimer = setInterval(async () => {
    try {
      const result = await api('/api/news/globe');
      const newData = result.countries || [];
      if (newData.length > 0) {
        updateGlobeSentiment(newData);
        const statusText = document.getElementById('globeStatusText');
        if (statusText) statusText.textContent = 'Live · ' + newData.length + ' countries · ' + (result.total_articles?.toLocaleString() || '0') + ' articles · ' + new Date().toLocaleTimeString();
      }
    } catch (e) { console.log('Sentiment poll failed, retrying...'); }
  }, 5000);
}

function stopSentimentPolling() {
  if (globeSentimentPollTimer) { clearInterval(globeSentimentPollTimer); globeSentimentPollTimer = null; }
}

function updateGlobeSentiment(newData) {
  if (!globeGroup || !globeCountryMeshes.length) return;
  const maxArticles = Math.max(...newData.map(d => d.articles || 0), 1);
  const dataMap = new Map();
  newData.forEach(d => dataMap.set(d.name, d));

  // Also update ripples
  const rippleDataMap = new Map();

  globeCountryMeshes.forEach(mesh => {
    if (!mesh.userData || !mesh.userData.name) return;
    const newD = dataMap.get(mesh.userData.name);
    if (!newD) return;
    mesh.userData.sentiment = newD.sentiment;
    mesh.userData.articles = newD.articles;
    mesh.userData.topic = newD.topic;
    mesh.userData.trend = newD.trend;
    mesh.userData.targetColor = new THREE.Color(sentimentColor(newD.sentiment));
    mesh.userData.targetSize = 0.6 + (newD.articles / maxArticles) * 2.5;
    mesh.userData.lastUpdate = performance.now();
    rippleDataMap.set(mesh.userData.name, sentimentColor(newD.sentiment));
  });

  // Update ripple colors to match new sentiment
  globeRipples.forEach(ripple => {
    // Find nearest country for this ripple (simplified: match by stored position)
    // Just update all ripples to a blended color for now
    // A more precise approach would store country name on ripple
  });
...
async function fetchGlobeData() {
  ...
  try {
    ...
    updateGlobeStats();
    if (loading) loading.classList.add('hidden');
    if (statusText) statusText.textContent = 'Demo mode · sample data · polling every 5s';
  }
}
...
function drawGlobeGrid(radius) {
  ...
  meridians.forEach(lon => {
    ...
    globeGroup.add(line); globeBoundaryLines.push(line);
  });
  const parallels = [-60, -30, 0, 30, 60];
  parallels.forEach(lat => {
    const pts = [];
    for (let lon = -180; lon <= 180; lon += 3) pts.push(latLonToVector3(lat, lon, radius));
    const geo = new THREE.BufferGeometry().setFromPoints(pts);
...
function drawCountryBoundaries(radius) {
  const coastlines = [
    ...
    // === SRI_LANKA ===
    [[9.8, 80.0], [9.5, 80.2], [8.5, 81.0], [7.0, 81.8], [6.0, 80.5], [5.8, 79.8], [6.5, 79.5], [7.5, 79.8], [8.5, 80.0], [9.8, 80.0]],
    // === CUBA ===
    [[23.2, -82.3], [23.0, -80.0], [22.0, -77.0], [20.0, -74.0], [19.8, -77.0], [20.0, -78.0], [21.5, -79.5], [22.5, -80.5], [23.2, -82.3]],
    // === HISPANIOLA ===
    [[20.0, -72.7], [19.7, -71.8], [18.2, -71.1], [18.0, -71.6], [18.0, -72.7], [18.4, -74.4], [19.9, -74.5], [20.0, -72.7]],
    // === PAPUA ===
    [[-2.5, 140.7], [-3.0, 141.0], [-5.0, 141.5], [-7.0, 141.0], [-8.5, 140.5], [-8.0, 139.5], [-6.0, 139.0], [-4.0, 139.5], [-2.5, 140.7]],
  ];


  // Warm gold/ivory dots — constellation aesthetic with continent tinting
  const dotGeo = new THREE.SphereGeometry(1.2, 6, 6);
  const dotMat = new THREE.MeshBasicMaterial({
    color: 0xfff8e7, transparent: true, opacity: 0.65,
    blending: THREE.AdditiveBlending, depthTest: true
  });
  const glowGeo = new THREE.SphereGeometry(2.2, 6, 6);
  const glowMat = new THREE.MeshBasicMaterial({
    color: 0xfff8e7, transparent: true, opacity: 0.08,
    blending: THREE.AdditiveBlending, depthTest: true, depthWrite: false
  });
  const lineMat = new THREE.MeshBasicMaterial({
    color: 0xc8b898, transparent: true, opacity: 0.15,
    blending: THREE.AdditiveBlending, depthTest: true, depthWrite: false
  });
...
function initGlobe() {
  const container = document.getElementById('newsGlobeContainer');
  if (!container) return;
  container.innerHTML = '';
  ...
  // Base globe — lighter for contrast
  const sphereGeo = new THREE.SphereGeometry(R, 64, 64);
  const sphereMat = new THREE.MeshBasicMaterial({
    color: 0x142a4a, transparent: true, opacity: 0.9, wireframe: false
  });
  ...
  canvas.addEventListener('mousemove', e => {
    ...
    if (hits.length > 0) {
      ...
      if (d) showGlobeTooltip(d, e.clientX + 15, e.clientY - 10);
...
function updateGlobePoints() {
  ...
  globeCountryMeshes = [];
  globeRipples.forEach(r => globeGroup.remove(r.mesh));
  globeRipples = [];
  ...
  globeData.forEach(d => {
    ...
    const sentimentIntensity = Math.abs(d.sentiment) / 60; // 0-1 based on sentiment magnitude

    // Main point — glowing sphere (type: 'core' for pulse targeting)
    ...
    mesh.userData = { ...d, type: 'core', baseSize: size, targetColor: new THREE.Color(color), lastUpdate: performance.now() };
    globeGroup.add(mesh);
    globeCountryMeshes.push(mesh);

    // Outer glow halo (type: 'halo' — follows core but doesn't pulse independently)
    const glowGeo = new THREE.SphereGeometry(size * 2.0, 16, 16);
    const glowMat = new THREE.MeshBasicMaterial({
    ...
    glowMesh.userData = { ...d, type: 'halo', baseSize: size * 2.0, targetColor: new THREE.Color(color), lastUpdate: performance.now() };
    globeGroup.add(glowMesh);
    globeCountryMeshes.push(glowMesh);

    // Ripple ring for ALL countries — intensity varies by article volume
    const rippleGeo = new THREE.RingGeometry(size * 1.8, size * 2.0, 32);
    const rippleMat = new THREE.MeshBasicMaterial({
      color: color, transparent: true, opacity: 0.25,
      side: THREE.DoubleSide, blending: THREE.AdditiveBlending, depthWrite: false
    });
    const ripple = new THREE.Mesh(rippleGeo, rippleMat);
    ripple.position.copy(pos.clone().multiplyScalar(1.002)); // Slightly above surface
    ripple.lookAt(new THREE.Vector3(0,0,0));
    globeGroup.add(ripple);
    // Faster ripples for high-activity, slower for low; phase offset by country
    const rippleSpeed = 0.3 + (d.articles / maxArticles) * 1.2;
    globeRipples.push({
      mesh: ripple, baseSize: size, speed: rippleSpeed,
      phase: Math.random() * Math.PI * 2,
      pos: pos // Store original position for reorientation
    });

    // Particle trail dots for high-sentiment countries (intense emotions = more particles)
    if (sentimentIntensity > 0.3) {
      const particleCount = Math.floor(sentimentIntensity * 6);
      for (let p = 0; p < particleCount; p++) {
        const pGeo = new THREE.SphereGeometry(size * 0.25, 4, 4);
        ...
        particle.position.copy(pos).add(offset);
        particle.userData = {
          ...d, type: 'particle', baseSize: size * 0.25,
          targetColor: new THREE.Color(color), lastUpdate: performance.now(),
          orbitRadius: size * (2 + Math.random() * 3),
        ...
        globeCountryMeshes.push(particle);
      }
    }

    // Spike for high volume
    if (d.articles > maxArticles * 0.35) {
      const spikeHeight = size * 4;
      const spikeGeo = new THREE.CylinderGeometry(size * 0.12, 0, spikeHeight, 8);
      const spikeMat = new THREE.MeshBasicMaterial({
        color: color, transparent: true, opacity: 0.4, blending: THREE.AdditiveBlending
      });
      ...
      spike.userData = { ...d, type: 'spike', lastUpdate: performance.now() };
...
function startGlobeAnimation() {
  ...
  function animate() {
    ...
    // Animate country meshes: smooth color transitions + type-specific effects
    globeCountryMeshes.forEach(mesh => {
      if (!mesh.userData || !mesh.material) return;

      // Smooth color transition
      if (mesh.userData.targetColor) {
        mesh.material.color.lerp(mesh.userData.targetColor, 0.06);
      }

      const type = mesh.userData.type;
      const baseSize = mesh.userData.baseSize || 1;
      const sentimentMag = Math.abs(mesh.userData.sentiment || 0) / 60;

      if (type === 'core') {
        // Heartbeat pulse: faster + stronger for high sentiment magnitude
        const pulseFreq = 2 + sentimentMag * 3; // 2-5 Hz
        const pulseAmp = 0.06 + sentimentMag * 0.1; // 6-16% amplitude
        const pulse = 1 + Math.sin(globePulseTime * pulseFreq + mesh.position.x * 3) * pulseAmp;
        mesh.scale.setScalar(pulse);
      } else if (type === 'halo') {
        // Halo breathes opposite to core for organic feel
        const breathe = 1 + Math.sin(globePulseTime * 1.5 + mesh.position.x * 3 + Math.PI) * 0.05;
        mesh.scale.setScalar(breathe);
        // Halo opacity pulses with sentiment
        mesh.material.opacity = 0.08 + sentimentMag * 0.15;
      } else if (type === 'particle') {
        // Particles orbit around their country center
        const ud = mesh.userData;
        const orbitAngle = globePulseTime * ud.orbitSpeed + ud.orbitPhase;
        const axis = ud.orbitAxis;
        const center = latLonToVector3(ud.lat, ud.lon, 100);
        // Create orbit offset using axis
        const perp1 = new THREE.Vector3(0,1,0).cross(axis).normalize();
        if (perp1.length() < 0.01) perp1.set(1,0,0);
        const perp2 = axis.clone().cross(perp1).normalize();
        const offset = perp1.clone().multiplyScalar(Math.cos(orbitAngle) * ud.orbitRadius)
          .add(perp2.clone().multiplyScalar(Math.sin(orbitAngle) * ud.orbitRadius * 0.6));
        mesh.position.copy(center).add(offset);
        // Fade particles in/out
        mesh.material.opacity = 0.3 + Math.sin(globePulseTime * 3 + ud.orbitPhase) * 0.3;
      } else if (type === 'spike') {
        // Spikes gently sway
        const sway = Math.sin(globePulseTime * 0.8 + mesh.position.x) * 0.02;
        mesh.rotation.z += sway * 0.01;
      }
    });

    // Animate ripple rings — expanding waves that fade
    globeRipples.forEach(ripple => {
      ripple.phase += 0.015 * ripple.speed;
      // Use sine squared for always-positive, smooth wave
      const wave = Math.sin(ripple.phase);
      const wavePos = (wave + 1) / 2; // 0 to 1
      const expand = 1 + wavePos * 0.8; // Expand up to 80%
      ripple.mesh.scale.setScalar(expand);
      // Opacity: peak at wave=0, fade at extremes
      ripple.mesh.material.opacity = 0.2 * (1 - wavePos) * (0.5 + 0.5 * Math.sin(ripple.phase * 2 + 1));
      // Reorient ripple to always face outward from globe center
