...
function updateGlobePoints() {
  ...
  globeData.forEach(d => {
    ...
    mesh.userData.targetColor = new THREE.Color(color);
    globeGroup.add(mesh);
    ...
    glowMesh.userData = d;
    glowMesh.userData.baseSize = size * 1.8;
    globeGroup.add(glowMesh);
    globeCountryMeshes.push(glowMesh);

    // Ripple ring for high activity
    if (d.articles > maxArticles * 0.2) {
      const rippleGeo = new THREE.RingGeometry(size * 2, size * 2.3, 32);
      ...
      globeRipples.push({ mesh: ripple, baseSize: size, speed: 0.5 + Math.random() * 0.5, phase: Math.random() * Math.PI * 2 });
    }
...
function startGlobeAnimation() {
  ...
  function animate() {
    ...
    // Animate country meshes: smooth color transitions + pulse
    globeCountryMeshes.forEach(mesh => {
      if (mesh.userData.targetColor && mesh.material) {
        const now = performance.now();
        const elapsed = (now - (mesh.userData.lastUpdate || now)) / 1000;
        const t = Math.min(1, elapsed * 2); // 0.5s transition
        mesh.material.color.lerp(mesh.userData.targetColor, 0.08);
        // Pulse effect
        const pulse = 1 + Math.sin(globePulseTime * 2 + mesh.position.x) * 0.08;
    ...
    globeRipples.forEach(ripple => {
      ripple.phase += 0.02 * ripple.speed;
      const expand = 1 + Math.sin(ripple.phase) * 0.5;
      ripple.mesh.scale.setScalar(expand);
      ripple.mesh.material.opacity = 0.3 * (1 - Math.sin(ripple.phase) * 0.5);
      ripple.mesh.rotation.z += 0.01;
    });
